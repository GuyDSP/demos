...
def plot_recorders(
    ...
    **layout_kwargs,
    ...
    """Plot multiple time-series or XY plots from recorder dataframes using Plotly.

    Parameters
    ----------
    recs : dict[str, pd.DataFrame]
        A dictionary mapping recorder names to their corresponding pandas DataFrames.
        Each DataFrame must contain columns to be used for plotting
        (e.g., time, variable values).

    pplots : Sequence[Sequence[tuple[str, str]]
        A grid layout definition for the subplots.
        Each inner sequence defines a row of plots, and each tuple (x, y) specifies which
        DataFrame columns to plot (x on the horizontal axis, y on the vertical).

    title_text : (str, optional)
        Title displayed at the top of the figure. Default is "CPU simulation analysis".

    **layout_kwargs :
        Additional keyword arguments passed to `fig.update_layout()`, such as height, width,
        and other layout properties.

    Returns
    -------
        None. The function displays the interactive Plotly figure directly.
    """

    rows = len(pplots)
    cols = max(len(row) for row in pplots)
    # Loop over each subplot index to apply grid styling; compute (row, col) from 1D index
